...
// TestWithBoardSync_NoOpWhenNilOrStatusEmpty verifies syncBoardStatusInProgress
// never calls the board API when boardSync is nil or inProgressStatus is empty.
func TestWithBoardSync_NoOpWhenNilOrStatusEmpty(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t.Errorf("unexpected request to %s — boardSync should be a no-op", r.URL.Path)
		http.Error(w, "unexpected", http.StatusInternalServerError)
	}))
	defer server.Close()

	client := NewClientWithBaseURL(testutil.FakeGitHubToken, server.URL)
	issue := &Issue{Number: 1, NodeID: "ISSUE_1"}

	t.Run("nil boardSync", func(t *testing.T) {
	...
	t.Run("empty inProgressStatus", func(t *testing.T) {
		bs := NewProjectBoardSync(client, &ProjectBoardConfig{
			Enabled:       true,
			ProjectNumber: 1,
			StatusField:   "Status",
		}, "org")
		poller, err := NewPoller(client, "org/repo", "pilot", 0, WithBoardSync(bs, ""))
		if err != nil {
			t.Fatalf("NewPoller: %v", err)
		}
		poller.syncBoardStatusInProgress(context.Background(), issue)
...
// TestWithBoardSync_UpdatesStatusOnDispatch verifies that a configured boardSync
// with a non-empty inProgressStatus actually drives the update-status GraphQL flow.
func TestWithBoardSync_UpdatesStatusOnDispatch(t *testing.T) {
	var updateCalled bool
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		var req GraphQLRequest
		if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
	...
	poller, err := NewPoller(client, "org/repo", "pilot", 0, WithBoardSync(bs, "In Dev"))
	if err != nil {
		t.Fatalf("NewPoller: %v", err)
	}

	poller.syncBoardStatusInProgress(context.Background(), &Issue{Number: 1, NodeID: "ISSUE_1"})
	if !updateCalled {
		t.Error("expected the field-update mutation to be called")
	}
}
