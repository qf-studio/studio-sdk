...
type Config struct {
	...
	ProjectPath       string                   `yaml:"project_path"`        // Required project path - must match repo
...
// StaleLabelCleanupConfig holds settings for auto-cleanup of stale trigger labels
...
type ProjectBoardConfig struct {
	...
	SourceEnabled bool            `yaml:"source_enabled"` // Source candidates from the board instead of by label
...
const (
	...
	LabelRetryReady         = "pilot-retry-ready"
	LabelTitleRejected      = "pilot-title-rejected"
	LabelSuperseded         = "pilot-superseded"
	LabelBlocked            = "pilot-blocked"
	LabelNeedsClarification = "pilot-needs-clarification"
...
type PullRequest struct {
	...
	User           *User  `json:"user,omitempty"` // PR author; populated by SearchOpenPRsForIssue
}
