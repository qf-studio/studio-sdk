...
// The callback receives a *WorkItem and must return a *core.IssueResult.
// *WorkItem → core.IssueEvent translation happens at the Adapter boundary (adapter.go).
...
// On startup, processed items are loaded from the store to prevent re-processing after restart.
...
func (p *Poller) processIssueAsync(ctx context.Context, item WorkItem) {
	...
	sanitizeWorkItemInPlace(&item, p.logger)
