...
// WebhookHandlerOption is a functional option for WebhookHandler
type WebhookHandlerOption func(*WebhookHandler)

// WithLogger sets the logger for the WebhookHandler
...
func NewWebhookHandler(client *Client, webhookSecret, pilotTag string, opts ...WebhookHandlerOption) *WebhookHandler {
	h := &WebhookHandler{
		client:        client,
...
func (h *WebhookHandler) Handle(ctx context.Context, payload *WebhookPayload) error {
	h.logger.Debug("Azure DevOps webhook received",
	...
	default:
		h.logger.Debug("Ignoring webhook event", slog.String("event", payload.EventType))
		return nil
	}
}
...
func (h *WebhookHandler) handleWorkItemCreated(ctx context.Context, payload *WebhookPayload) error {
	...
	if !h.hasPilotTag(resource.Fields) {
		h.logger.Debug("Work item does not have pilot tag, skipping",
	...
	if !h.isAllowedWorkItemType(resource.Fields) {
		h.logger.Debug("Work item type not in allowed list, skipping",
			slog.Int("id", resource.ID))
		return nil
	}
...
func (h *WebhookHandler) handleWorkItemUpdated(ctx context.Context, payload *WebhookPayload) error {
	...
	if !h.hasPilotTag(currentFields) {
		h.logger.Debug("Work item does not have pilot tag after update, skipping",
			slog.Int("id", resource.ID))
		return nil
	}
	...
	if h.hasTag(currentFields, TagInProgress) || h.hasTag(currentFields, TagDone) {
		h.logger.Debug("Work item already processed, skipping",
			slog.Int("id", resource.ID))
		return nil
	}
...
func (h *WebhookHandler) processWorkItem(ctx context.Context, workItemID int) error {
	...
	h.logger.Info("Fetched work item details",
