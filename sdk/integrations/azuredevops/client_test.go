...
func TestGetWorkItem(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		_, _ = w.Write(mustMarshal(t, workItem))
	}))
	defer server.Close()
...
func TestListWorkItemsByWIQL(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if callCount == 1 {
			// WIQL query
			_, _ = w.Write(mustMarshal(t, wiqlResponse))
		} else {
			// Work items batch
			_, _ = w.Write(mustMarshal(t, workItemsResponse))
		}
	}))
	defer server.Close()
...
func TestAddWorkItemTag(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if callCount == 1 {
			...
			_, _ = w.Write(mustMarshal(t, updatedWorkItem))
		}
	}))
	defer server.Close()
...
func TestCreatePullRequest(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write(mustMarshal(t, expectedPR))
	}))
	defer server.Close()
