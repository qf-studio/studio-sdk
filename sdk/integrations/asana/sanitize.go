...
// sanitizeTaskInPlace strips invisible Unicode format characters from the
// task's untrusted text fields (Name, Notes, HTMLNotes) before the task is
// handed to any downstream consumer. Emits a slog.Warn when any runes are
// stripped — attack-in-progress signal.
func sanitizeTaskInPlace(task *Task, logger *slog.Logger) {
	if task == nil {
		return
	}
