...
// NewClient creates a new Discord REST client.
...
// On 429 responses it reads Retry-After and retries up to maxRetries times.
func (c *Client) doRequest(ctx context.Context, method, endpoint string, body interface{}) ([]byte, error) {
	for attempt := 0; attempt <= c.maxRetries; attempt++ {
		...
		if statusCode == http.StatusTooManyRequests {
			...
			c.log.Warn("discord rate limited",
...
func (c *Client) doRequestOnce(ctx context.Context, method, endpoint string, body interface{}) ([]byte, int, time.Duration, error) {
	...
	req.Header.Set("User-Agent", "DiscordBot (studio-sdk, 1.0)")
...
// Discord sends this as fractional seconds.
...
// SendMessageWithComponents sends a message with interactive components (buttons via action rows).
...
func (c *Client) CreateInteractionResponse(ctx context.Context, interactionID, interactionToken string, responseType int, content string) error {
	...
	_, err := c.doRequest(ctx, http.MethodPost,
		fmt.Sprintf("/interactions/%s/%s/callback", interactionID, interactionToken), payload)
	if err != nil {
